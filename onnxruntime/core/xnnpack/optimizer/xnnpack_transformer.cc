...
Status XNNPackTransformer::ApplyImpl(Graph& main_graph, bool& modified, int /* graph_level */,
  ...
  for (auto& nodeRef : gv.Nodes()) {
    if (!NhwcTransformer::IsConvSupportedByXNNPack(nodeRef, false) &&
        !NhwcTransformer::IsMaxPoolSupportedByXNNPack(nodeRef, false))
      continue;
  ...
  for (NodeIndex ni : maxpool_nodes) {
    ...
    Node& new_node =
        main_graph.AddNode(node_p->Name(), "XnnPackMaxPooling2d", "", {}, {}, nullptr, "com.microsoft.xnnpack");
    ...
    ORT_RETURN_IF_ERROR(MoveInputOutput(main_graph, nodeRef, new_node, move_info, false));
    ValueMoveInfo move_info2(ArgType::kOutput, ArgType::kOutput);
    ORT_RETURN_IF_ERROR(MoveInputOutput(main_graph, nodeRef, new_node, move_info2, false));
